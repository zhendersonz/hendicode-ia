# -*- coding: utf-8 -*-
"""Baixa apenas datasets faltantes. Nao redownloada o que ja existe."""
...
LOG = os.path.join(OUTPUT_DIR, "log_restante.txt")
...
def ja_existe(nome):
    path = os.path.join(OUTPUT_DIR, f"{nome}.txt")
    if os.path.exists(path) and os.path.getsize(path) > 1000:
        log(f"  [JA EXISTE] {nome}: {os.path.getsize(path)/1e6:.1f} MB")
        return True
    return False

def extract_parquet(url, nome_saida, colunas, max_mb=500):
    if ja_existe(nome_saida):
        return 0
    temp = os.path.join(OUTPUT_DIR, "_temp.parquet")
    log(f"  Baixando {nome_saida}...")
    ok = curl_dl(url, temp, timeout=1800)
    if not ok:
        log(f"  FALHA download")
        return 0
    fsize = os.path.getsize(temp)
    log(f"  Baixado: {fsize/1e6:.0f} MB")
    if fsize > max_mb * 1024 * 1024:
        log(f"  > limite {max_mb}MB, ignorando")
        os.remove(temp)
        return 0
    try:
...
def download_json(ds_id, arq, nome_saida, cols):
    if ja_existe(nome_saida):
        return 0
    url = f"https://huggingface.co/datasets/{ds_id}/resolve/main/{arq['rfilename']}"
    temp = os.path.join(OUTPUT_DIR, "_temp.json")
    ok = curl_dl(url, temp)
    if not ok:
        return 0
    try:
...
# ==================== EXECUCAO ====================
log("="*60)
log("DOWNLOAD RESTANTE - datasets faltantes")
log("="*60)

total_chars = 0

# 1. CSN - ruby + php + all
log("\n--- code_search_net ruby ---")
sis = get_siblings("code-search-net/code_search_net")
for lang in ["ruby", "php"]:
...
# 2. CODEPARROT (.json.gz)
log("\n--- codeparrot-clean-train ---")
sis_cp = get_siblings("codeparrot/codeparrot-clean-train")
gz_files = [s for s in sis_cp if s["rfilename"].endswith(".json.gz")]
...
# 3. RSTAR-CODER
log("\n--- rStar-Coder ---")
sis_rs = get_siblings("microsoft/rStar-Coder")
parqs_rs = [s for s in sis_rs if s["rfilename"].endswith(".parquet")]
for arq in parqs_rs[:10]:
...
# 4. CC100 PORTUGUES
log("\n--- cc100 lang=pt ---")
sis_cc = get_siblings("statmt/cc100")
pt_files = [s for s in sis_cc if "lang=pt" in s["rfilename"] and s["rfilename"].endswith(".parquet")]
...
for ds_id, cols, nome in inst_list:
    log(f"\n  {ds_id}")
    if ja_existe(nome):
        continue
    sis_i = get_siblings(ds_id)
    parqs = [s for s in sis_i if s["rfilename"].endswith(".parquet")]
    if parqs:
        for arq in parqs[:2]:
            url = f"https://huggingface.co/datasets/{ds_id}/resolve/main/{arq['rfilename']}"
            ...
            total_chars += c
        continue
    jsons = [s for s in sis_i if s["rfilename"].endswith(".json")]
    for arq in jsons[:2]:
        c = download_json(ds_id, arq, nome, cols)
        total_chars += c

# 6. GITHUB-CODE-2025 (1 arquivo)
log("\n--- github-code-2025 (1/2) ---")
if not ja_existe("github_code_2025_1"):
    url_g1 = "https://huggingface.co/datasets/nick007x/github-code-2025/resolve/main/above-2-stars/train_000.parquet"
    c = extract_parquet(url_g1, "github_code_2025_1", ["content"], 3000)
    total_chars += c

log("\n--- github-code-2025 (2/2) ---")
if not ja_existe("github_code_2025_2"):
    url_g2 = "https://huggingface.co/datasets/nick007x/github-code-2025/resolve/main/above-2-stars/train_001.parquet"
    c = extract_parquet(url_g2, "github_code_2025_2", ["content"], 3000)
    total_chars += c
